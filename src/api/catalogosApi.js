...
export const consultaMotivos = (tipo, mostrar_cargando = true, mostrar_cargado = true, mostrar_error = true) => {
    cargando();
    return API.post(`${ruta_base}getMotivos`, {tipo})
        .then(res => mostrar_cargado ? returnGuardadoCorrecto(res.data) : res.data)
        .catch(err => mostrar_error ? mostrarErrorGuardar(err) : err);
};
export const consultaParametros = (campo, mostrar_cargando = true, mostrar_cargado = true, mostrar_error = true) => {
    cargando();
    return API.post(`${ruta_base}getParametros`, {campo})
        .then(res => mostrar_cargado ? returnGuardadoCorrecto(res.data) : res.data)
        .catch(err => mostrar_error ? mostrarErrorGuardar(err) : err);
};