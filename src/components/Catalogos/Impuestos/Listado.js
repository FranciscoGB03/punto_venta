                    ...
                    <table className=" table table-condensed table-hover ">
                        ...
                        {(registros || []).map((reg, idx) => <tr key={idx}>
                            ...
                            <td className="text-right">
                                <NumberFormat displayType="text" value={reg.porcentaje} suffix="%" decimalScale={2} fixedDecimalScale={true}/>
                            </td>
                            <td className="text-right">{reg.cantidad}</td>
                            <td className="text-right">{moment(reg.vigenciaInicio).format('DD/MM/YYYY')}</td>
                            <td className="text-right">{reg.vigenciaFin?moment(reg.vigenciaFin).format('DD/MM/YYYY'):''}</td>
    ...
    return (
        ...
        <tr>
            ...
            <th className='text-center cursor'
                ...
                <IconosSort key_sort={'porcentaje'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:porcentaje')}
            </th>
            <th className='text-center cursor'
                onClick={() => handleSort('cantidad')}>
                <IconosSort key_sort={'cantidad'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:cantidad')}
            </th>
            <th className='text-center cursor'
                onClick={() => handleSort('vigenciaInicio')}>
                <IconosSort key_sort={'vigenciaInicio'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:vigenciaInicio')}
            </th>
            <th className='text-center cursor'
                onClick={() => handleSort('vigenciaFin')}>
                <IconosSort key_sort={'vigenciaFin'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:vigenciaFin')}
            </th>
            <th className='text-center cursor'
...
const TheadBuscador = ({filtro, setFiltro}) => {
    return (
        ...
        <tr>
            ...
            <th className='py-1'>
                <input className='form-control form-control-sm' value={filtro.porcentaje || ""}
                       placeholder={i18next.t('catalogos:buscar')}
                       onChange={(e) => setFiltro({...filtro, porcetaje: e.target.value})}/>
            </th>
            <th className='py-1'>
                <input className='form-control form-control-sm' value={filtro.cantidad || ""}
                       placeholder={i18next.t('catalogos:buscar')}
                       onChange={(e) => setFiltro({...filtro, cantidad: e.target.value})}/>
            </th>
            <th></th>
            <th></th>
            <th></th>
