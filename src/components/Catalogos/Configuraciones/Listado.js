    ...
    return (
        ...
        <tr>
            <th className='text-center cursor'
                onClick={() => handleSort('tipo_configuracion.nombre')}>
                <IconosSort key_sort={'tipo_configuracion.nombre'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:tipoConfiguracion')}
            </th>
            <th className='text-center cursor'
...
const TheadBuscador = ({filtro, setFiltro}) => {
    return (
        ...
        <tr>
            <th className='py-1'>
                <input className='form-control form-control-sm' value={filtro.tipo_configuracion?.nombre || ""}
                       placeholder={i18next.t('catalogos:buscar')}
                       onChange={(e) => setFiltro({...filtro, tipo_configuracion:{nombre: e.target.value}})}/>
            </th>
            <th className='py-1'>
