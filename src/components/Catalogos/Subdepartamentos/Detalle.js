...
import { guardaSubdepartamento } from '../../../api/subdepartamentosApi';

const Detalle = ({seleccionado,catalogo, onGuardar}) => {
    //|------Hooks/Constantes------|//
    ...
    const guardarRegistro = () => {
        guardaSubdepartamento(registro)
            .then(res => {
                cerrarAlert();
                onGuardar(registro);
            })
        ...
        <div className="card border-tabla z-depth-2">
            ...
            <div className="m-2">
                ...
                <div className="d-flex mb-2 mt-1">
                    <span className="py-0 pr-3">{i18next.t('catalogos:departamento')}:</span>
                    <Select onSelect={e => setRegistro({...registro, departamento: e ? e:{id:''}})}
                            className={`${registro.departamento?.id===undefined||registro.departamento?.id==='' ? 'is-invalid' : ''}`}
                            selected={registro.departamento|| {id:''}}
                            options={catalogo}
                            labelKey="nombre"/>
                </div>
                <div className="d-flex mb-2 mt-1">
