                ...
                <div id="div_tabla" className="table-responsive">
                    <table className=" table table-condensed table-hover ">
                        ...
                        {(registros || []).map((reg, idx) =>
                            <tr key={idx}>
                                <td>{reg.codigoBarras}</td>
                                <td>{reg.nombre}</td>
                                <td>{reg.presentacion?.nombre}</td>
                                <td>{reg.subDepartamento?.nombre}</td>
    ...
    return (
        ...
        <tr>
            <th className='text-center cursor'
                onClick={() => handleSort('codigoBarras')}>
                <IconosSort key_sort={'codigoBarras'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:codigoBarras')}
            </th>
            <th className='text-center cursor'
            ...
            <th className='text-center cursor'
                onClick={() => handleSort('presentacion.nombre')}>
                <IconosSort key_sort={'presentacion.nombre'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:presentacion')}
            </th>
            <th className='text-center cursor'
                onClick={() => handleSort('subDepartamento.nombre')}>
                <IconosSort key_sort={'subDepartamento.nombre'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:subdepartamento')}
            </th>
            <th className='text-center cursor'
                onClick={() => handleSort('proveedor.nombre')}>
                <IconosSort key_sort={'proveedor.nombre'} key_actual={sort.key} order={sort.order}/>
                {i18next.t('catalogos:proveedor')}
            </th>
            <th className='text-center cursor'
...
const TheadBuscador = ({filtro, setFiltro}) => {
    return (
        ...
        <tr>
            <th className='py-1'>
                <input className='form-control form-control-sm' value={filtro.codigoBarras || ""}
                       placeholder={i18next.t('catalogos:buscar')}
                       onChange={(e) => setFiltro({...filtro, codigoBarras: e.target.value})}/>
            </th>
            <th className='py-1'>
            ...
            <th className='py-1'>
                <input className='form-control form-control-sm' value={filtro.presentacion || ""}
                       placeholder={i18next.t('catalogos:buscar')}
                       onChange={(e) => setFiltro({...filtro, presentacion: e.target.value})}/>
            </th>
            <th className='py-1'>
                <input className='form-control form-control-sm' value={filtro.subDepartamento || ""}
                       placeholder={i18next.t('catalogos:buscar')}
                       onChange={(e) => setFiltro({...filtro, subDepartamento: e.target.value})}/>
            </th>
            <th></th>
        </tr>
        </thead>);
