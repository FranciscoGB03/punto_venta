...
import { guardaProducto } from '../../../api/productoApi';

const Detalle = ({seleccionado, onGuardar,presentaciones,proveedores,subdepartamentos}) => {
    //|------Hooks/Constantes------|//
    ...
    return (
        <div className="card border-tabla z-depth-2">
            ...
            <div className="m-2">
                <Campo etiqueta={`${i18next.t('catalogos:codigoBarras')}: `}
                       data="codigoBarras"
                       containerClass="d-flex justify-content-start"
                       inputClass={`form-control form-control-sm ml-1 ${!registro.codigoBarras ? 'is-invalid' : ''}`}
                       value={registro.codigoBarras || ''}
                       autoFocus={true}
                       maxLength={50}
                       type="number"
                       onChange={e => setRegistro({...registro, codigoBarras: e.target.value})}/>
                ...
                <div className="d-flex mb-2 mt-1">
                    <span className="py-0 pr-3">{i18next.t('catalogos:presentacion')}:</span>
                    <Select onSelect={e => setRegistro({...registro, presentacion: e ? e : {}})}
                            className={`${registro.presentacion?.id===undefined||registro.presentacion?.id==='' ? 'is-invalid' : ''}`}
                            selected={registro.presentacion||{}}
                            options={presentaciones}
                            labelKey="nombre"/>
                </div>
                <div className="d-flex mb-2 mt-1">
                    <span className="py-0 pr-3">{i18next.t('catalogos:subdepartamento')}:</span>
                    <Select onSelect={e => setRegistro({...registro, subDepartamento: e ? e : {}})}
                            className={`${registro.subDepartamento?.id===undefined||registro.subDepartamento?.id==='' ? 'is-invalid' : ''}`}
                            selected={registro.subDepartamento||{}}
                            options={subdepartamentos}
                            labelKey="nombre"/>
                </div>
                <div className="d-flex mb-2 mt-1">
                    <span className="py-0 pr-3">{i18next.t('catalogos:presentacion')}:</span>
                    <Select onSelect={e => setRegistro({...registro, proveedor: e ? e : {}})}
                            className={`${registro.proveedor?.id===undefined||registro.proveedor?.id==='' ? 'is-invalid' : ''}`}
                            selected={registro.proveedor||{}}
                            options={proveedores}
                            labelKey="nombre"/>
                </div>
                <div className="d-flex mb-2 mt-1">
